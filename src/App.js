...
function App() {
  ...
  useEffect(()=>{
    const handleVerify = async () => {
      try {
        const resp = await API.get('/logged_in', {Origin: "https://bloomingtonwdr.com",withCredentials: true})
        setLoaded(true)
  ...
  const handleLogout = () => {
    API.delete("/logout", {Origin: "https://bloomingtonwdr.com",withCredentials: true})
    .then(response => {
      setCurrentUser(null);
    })
    .catch(error => {
